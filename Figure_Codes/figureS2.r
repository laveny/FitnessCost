...
p.A  <- draw.fn%>%
  ...
  labs(x=expression("Relative growth at 0"*mu*"g/mL"),
       y=expression("Relative growth at 2"*mu*"g/mL"),
       title = 'A')+
...
p.B <- draw.fn%>%
  ...
  scale_fill_manual(values = c('SUM' = '#32373E','count' = '#F74831'), labels = c('all variants', 'costless resistant variants'),
                    name = '')+
  labs(y='Count', x = "Type of variants", title = 'B')+
...
p.C <- plot_spacer() + p.C.above + plot_layout(heights = c(2,1)) 
